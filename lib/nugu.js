...
const OPTIONS = {
  // Nyuu's ignore `filename` option when source files are not from disk,
  // So we use a function instead, which is called when we handle the
  // files from rclone and set up procjson.
  /** @type {(File) => string} */
  "filename": ({ Name }) => Name,
...
module.exports = async function nugu(sourcePath, options = {}) {
  ...
  // `filename` option expects a function, which takes a File and returns a
  // string file name. The `filename` option can be a string with placeholders
  // for the File's properties.
  if (typeof filename === "string") {
    options.filename = function({AbsolutePath, Hashes = {}, ...file}) {
      const replacement = {
        ...file, // All main properties of file.
        ...Hashes, // Spreads out the hashes
        filename: AbsolutePath, // nyuu's for full file name and path as specified in command supplied
        basename: file.Name, // Base file name without path
        pathname: dirname(AbsolutePath),
      ...
      return filename.replace(
        /{(\w+)}/g,
        (placeholder, key) =>
          replacement.hasOwnProperty(key) ?
  ...
  const lsOptions = {};
  // If `filename` option contains any of the supported hashes, then we ask
  // rclone to return the hashes for the files.
  if (typeof filename === "string") {
    const hashes = [];
    ["crc32", "md5", "sha1"].forEach(hashType => {
      if (filename.indexOf(`{${hashType}}`) > -1) {
        hashes.push(hashType);
      }
    });

    if (hashes.length) {
      lsOptions["hash-type"] = hashes;
    }
  }
  const files = await lsjson(sourcePath, lsOptions);
